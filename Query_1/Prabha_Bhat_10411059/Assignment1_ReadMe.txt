...
-------------------------------
Step 1: Copy the myQuery1.java fiel to your local system.
...
if you are running it using command prompt compile the program using "javac myQuery1.java"
then run it using "java myQuery1"
...
while (result set having next element){
	...
	for (every entry in an ArrayList){
	  if (check for customer, product combination){
	   calculate sum and occurrence of the customer, product combination and
	   ...
	   come out of for loop if either maximum or minimum condition satisfies.
	  }
	}
 ...
 for (every 14 elements in an ArrayList){
  calculate the Average quantity for each customer product combination
